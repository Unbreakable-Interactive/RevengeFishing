Shader "Custom/URP/ISTImprovedsWater"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // (opcional) permite compilar más bajo en móviles
            ...
            // ========= COMPAT LAYER: StructuredBuffer (Metal/Vulkan) vs Uniform Array (GLES) =========
            #define MAX_WAVE_PARTICLES 64

            // Unity define flags por backend; estas cubren Metal y Vulkan
            #if defined(SHADER_API_METAL) || defined(SHADER_API_VULKAN)
                #define HAS_STRUCTURED_BUFFER 1
            #else
                #define HAS_STRUCTURED_BUFFER 0
            #endif

            #if HAS_STRUCTURED_BUFFER
                StructuredBuffer<float4> _WaveParticles;   // xy = position, z = amplitude, w = wavelength
            #else
                uniform float4 _WaveParticlesArr[MAX_WAVE_PARTICLES];
            #endif

            int _WaveParticleCount;

            float4 ReadWaveParticle(int i)
            {
            #if HAS_STRUCTURED_BUFFER
                return _WaveParticles[i];
            #else
                return _WaveParticlesArr[i];
            #endif
            }
            // =========================================================================================
            ...
            float3 CalculateDynamicWaves(float3 worldPosition)
            {
                ...
                int count = min(_WaveParticleCount, MAX_WAVE_PARTICLES);

                [loop]
                for (int i = 0; i < count; i++)
                {
                    float4 wp = ReadWaveParticle(i);
                    float2 particlePos = wp.xy;
                    float amplitude   = wp.z;
                    float wavelength  = wp.w;
