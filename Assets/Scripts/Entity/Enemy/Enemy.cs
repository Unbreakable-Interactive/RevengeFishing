...
public abstract class Enemy : Entity
{
    ...
    public virtual void SetPowerLevel(int newPowerLevel)
    {
        ...
        entityFatigue._maxFatigue = _powerLevel;
        entityFatigue._fatigue = 0; // Reset fatigue when power level changes
    ...
    public void TakeFatigue(int playerPowerLevel)
    {
        ...
        // 5% more fatigue
        entityFatigue._fatigue += (int)((float)playerPowerLevel * .05f);

        // Check if enemy should be defeated
        if (entityFatigue._fatigue >= entityFatigue._maxFatigue && _state == EnemyState.Alive)
        {
            TriggerDefeat();
        }
